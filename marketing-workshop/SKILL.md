...
description: Runs a founder or marketer through a product's marketing, end to end — positioning (Ries and Trout's method: the ladder, the leader's word, the hole, the one word and sentence, voice), copy (homepage, landing pages, ads, email, SEO), distribution (channels, ranking, 30-day plan), research, measurement and UTM tracking, automated execution where tools exist, scheduled monitoring, launch verification, and weekly review. Use whenever someone wants help with positioning, value proposition, homepage or ad copy, finding customers, channel strategy, tracking setup, launch readiness, or weekly review — or asks to "run the positioning flow", "do the copy flow", "build my marketing pipeline", "run the whole thing", "run the 3-hour workshop", or names the Coffee with Claude workshop.
---
