...
```sh
cp cofounder/flows/_TEMPLATE.md cofounder/flows/pricing.md
# write it
python3 cofounder/scripts/build_index.py       # SKILL.md's generated blocks
python3 cofounder/portable/build.py            # the paste file
python3 cofounder/scripts/build_skill_zip.py   # the ChatGPT Skills zip
```

`make -C cofounder` runs all three if you'd rather not remember the order.
...
```sh
python3 cofounder/scripts/build_index.py --check
python3 cofounder/portable/build.py --check
python3 cofounder/scripts/build_skill_zip.py --check
python3 -m unittest discover -s cofounder/tests
```
